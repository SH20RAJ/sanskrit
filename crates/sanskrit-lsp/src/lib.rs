...
impl SanskritLanguageServer {
    ...
    pub fn get_hover(&self, word: &str) -> Option<LspHover> {
        let doc = match word {
            ...
            "विफल" | "panic" => "```sanskrit\nविफल(सन्देश: सूत्र)\n```\nAborts execution with an explicit panic and diagnostic message.",
            "निश्चय" | "assert" => "```sanskrit\nनिश्चय(शर्त: तर्क, सन्देश: सूत्र)\n```\nValidates boolean condition; triggers panic if false.",
            "सूत्र" | "str" => "```sanskrit\nसूत्र\n```\nUTF-8 immutable zero-copy string slice.",
            "तर्क" | "bool" => "```sanskrit\nतर्क\n```\nBoolean logical type (सत्यम् / true, असत्यम् / false).",
            "पूर्णाङ्क" | "i32" => "```sanskrit\nपूर्णाङ्क\n```\n32-bit signed two's complement integer.",
    ...
    pub fn get_completions(&self) -> Vec<LspCompletionItem> {
        vec![
            ...
            LspCompletionItem {
                label: "विफल".to_string(),
                detail: "Panic handler (Devanagari)".to_string(),
                documentation: "विफल(\"त्रुटि:\");".to_string(),
                insert_text: Some("विफल(\"${1:सन्देश}\")".to_string()),
            },
            LspCompletionItem {
                label: "panic".to_string(),
                detail: "Panic handler (ASCII)".to_string(),
                documentation: "panic(\"error\");".to_string(),
                insert_text: Some("panic(\"${1:message}\")".to_string()),
            },
            LspCompletionItem {
                label: "निश्चय".to_string(),
                detail: "Assert condition (Devanagari)".to_string(),
                documentation: "निश्चय(शर्त, \"सन्देश\");".to_string(),
                insert_text: Some("निश्चय(${1:शर्त}, \"${2:सन्देश}\")".to_string()),
            },
            LspCompletionItem {
                label: "assert".to_string(),
                detail: "Assert condition (ASCII)".to_string(),
                documentation: "assert(condition, \"message\");".to_string(),
                insert_text: Some("assert(${1:condition}, \"${2:message}\")".to_string()),
            },
            LspCompletionItem {
                label: "सूत्र".to_string(),
                detail: "String slice type (Devanagari)".to_string(),
                documentation: "सूत्र (UTF-8 string slice)".to_string(),
                insert_text: Some("सूत्र".to_string()),
            },
            LspCompletionItem {
                label: "तर्क".to_string(),
                detail: "Boolean type (Devanagari)".to_string(),
                documentation: "तर्क (bool: सत्यम् / असत्यम्)".to_string(),
                insert_text: Some("तर्क".to_string()),
            },
        ]
    }
